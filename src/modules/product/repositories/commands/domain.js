...
class Product {
  ...
  async postProduct(payload) {
    ...
    const document = { id: uuid(), name, category, price, createdat: new Date().toISOString(), modifiedat: new Date().toISOString() };
  ...
  async updateProduct(payload) {
    ...
    if(!name && !category && !price) return wrapper.error(new ConflictError('no data to update'));
    ...
    if (result.err) {
      console.log(ctx, 'updateProduct', 'can not update to database', result.err);
      return wrapper.error(new InternalServerError('can not update to database'));
    }
  ...
  async deleteProduct(payload) {
    ...
    if (result.err) {
      console.log(ctx, 'deleteProduct', 'can not delete from database', result.err);
      return wrapper.error(new InternalServerError('can not delete from database'));
    }
